  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-serif text-2xl font-bold">🏢 Clients Assistant</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            {clients.length} entreprise{clients.length !== 1 ? 's' : ''} · {clients.filter(c => c.status === 'active').length} abonnement{clients.filter(c => c.status === 'active').length !== 1 ? 's' : ''} actif{clients.filter(c => c.status === 'active').length !== 1 ? 's' : ''}
          </p>
        </div>
      ...
      <Card className="border-blue-500/25 bg-blue-500/5">
        <CardContent className="p-4 text-xs text-muted-foreground leading-relaxed">
          Ce carnet de clients est distinct de la "Gestion ERP" de BizKey Sourcing — il suit les entreprises abonnées à l'assistant WhatsApp, leur formule et leur numéro connecté, séparément des clients d'importation.
        </CardContent>
      </Card>
      ...
      <div className="flex flex-col sm:flex-row gap-2.5">
        <div className="relative flex-1">
          ...
          <Input placeholder="Rechercher une entreprise..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9 h-10" />
        </div>
        <select
          value={statusFilter}
      ...
      {loading ? (
        ...
        <div className="py-16 text-center">
          ...
          {clients.length === 0 && <Button onClick={openCreate} size="sm" className="mt-3 rounded-full"><Plus className="h-4 w-4 mr-1" />Ajouter un client</Button>}
        </div>
      ) : (
