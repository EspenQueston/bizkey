...
const STATUS_META: Record<WhatsAppNumberStatus, { label: string; color: string; icon: string }> = {
  pending:  { label: 'En attente de validation', color: 'bg-amber-500/15 text-amber-600',  icon: '⏳' },
  active:   { label: 'Actif',                    color: 'bg-blue-500/15 text-blue-600',    icon: '✅' },
  inactive: { label: 'Inactif',                  color: 'bg-muted text-muted-foreground',  icon: '⏸️' },
}
...
export default function WhatsAppNumbersPage() {
  ...
  async function handleDelete(id: string) {
    if (!confirm('Supprimer ce numéro WhatsApp ?')) return
    setDeletingId(id)
    try {
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-serif text-2xl font-bold">📱 Numéros WhatsApp</h1>
          <p className="text-sm text-muted-foreground mt-0.5">{numbers.length} numéro{numbers.length !== 1 ? 's' : ''} configuré{numbers.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      <Card className="border-blue-500/25 bg-blue-500/5">
        <CardContent className="p-4 text-xs text-muted-foreground leading-relaxed">
          Connecter un vrai numéro nécessite un compte WhatsApp Business API (Meta). En attendant, un numéro peut être créé ici en statut "En attente" — l'assistant reste testable via le simulateur dans Conversations, indépendamment du statut de connexion réel.
        </CardContent>
      </Card>
      ...
      {loading ? (
        ...
        <div className="py-16 text-center">
          ...
          <p className="text-sm font-medium">Aucun numéro configuré</p>
          <Button onClick={openCreate} size="sm" className="mt-3 rounded-full"><Plus className="h-4 w-4 mr-1" />Ajouter un numéro</Button>
        </div>
      ) : (
              ...
              <Card key={n.id}>
                <CardContent className="p-4 space-y-2.5">
                  ...
                  {n.business_account_id && (
                    <p className="text-[10px] text-muted-foreground truncate">ID compte : {n.business_account_id}</p>
                  )}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-background rounded-2xl border border-border shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b border-border">
              <h2 className="font-serif font-bold">{editing ? 'Modifier le numéro' : 'Ajouter un numéro'}</h2>
              <button onClick={() => setShowModal(false)}><X className="h-5 w-5 text-muted-foreground" /></button>
            </div>
            <div className="p-5 space-y-3">
              <div className="space-y-1.5">
                <Label>Libellé *</Label>
                <Input placeholder="Ex: Support client Bénin" value={form.label} onChange={e => setForm(f => ({ ...f, label: e.target.value }))} className="h-10" />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                <Label>ID compte WhatsApp Business (optionnel)</Label>
